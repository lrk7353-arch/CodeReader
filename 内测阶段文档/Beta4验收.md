...
当前仓库工作目录是 `Z:\home\konglingrui\CodeReader`，实际指向 `\\wsl$\Ubuntu\...` UNC 路径。在此 UNC 路径下直接跑 `npm run cargo:test` 等 Node 脚本时，`cmd.exe` 不支持 UNC 作为 cwd，会退回 `C:\Windows`，导致脚本找不到。因此：

- Windows 侧门禁**不能**在 UNC 工作目录下直接跑 `npm run cargo:*`。
- 可靠的 Windows 门禁验证方式：把仓库克隆/映射到本地盘（如 `C:\CodeReader`）再跑，或通过 WSL 跑 Linux 门禁（已验证通过）。
- Linux 侧 `npm run verify:linux -- --json` 是当前最可靠的全量门禁证据，已通过 7 道门禁。
