...
```bash
git tag -a v1.0.0-rc.6 -m "CodeReader 1.0.0-rc.6"
...
Package smoke and the complete product journey are separate gates. After creating the immutable candidate tag, run the full journey on native Windows and Linux x64/ARM64 hardware. The package, candidate identity, examples, and historical seed data come from that immutable tag; the journey drivers and verifier come from a separately recorded audited workflow commit. `harnessCommitSha` identifies the measurement harness and is not a product-version claim. Generate one template per target and replace `pending` only with directly observed results:

```bash
node scripts/release-evidence.mjs journey-template --platform linux --arch x64 --tag v1.0.0-rc.6 --sha <40-character-candidate-commit> --harness-sha <40-character-audited-harness-commit> --output docs/release/evidence/v1.0.0-rc.6/native-journey-linux-x64.json
node scripts/release-evidence.mjs verify-journeys --input docs/release/evidence/v1.0.0-rc.6 --tag v1.0.0-rc.6 --sha <40-character-candidate-commit> --harness-sha <40-character-audited-harness-commit>
```
...
Publication is only performed by `.github/workflows/publish.yml`. Dispatch it with the existing immutable tag and the audited harness commit recorded by all four journey assets. The single write-capable job first pauses for maintainer approval in the protected `production-release-publish` environment. After approval it validates both identities, checks out the protected main workflow commit with complete history and the exact candidate tag separately, and proves the recorded harness commit is an ancestor of that main commit. It never checks out or executes code from the evidence-supplied harness SHA. The current main verifier then reconfirms the matching draft, downloads exactly four current `native-journey-*.json` assets, and validates both SHAs. Only the immediately following step may publish that same draft. This post-approval revalidation prevents evidence or draft state changed during approval from being trusted. Keep the environment restricted to required reviewers. Do not grant another workflow or local script permission to change the draft state.
