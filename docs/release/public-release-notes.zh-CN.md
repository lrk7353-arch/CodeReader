...
9. 如果正文提到项目地图、掌握度、个人记录或变化复查，当前制品是否已在维护者真实项目旅程中验收？未完成时必须写成候选或已知限制，不能用自动化测试代替。
...
“一分钟内获得解释”“五分钟理解项目”“恢复率 95%”或“陈旧识别率 80%”只有在对应 Release 保存样本、环境、失败和统计分母后才能引用；否则它们只是内部验证目标，不是 SLA。
